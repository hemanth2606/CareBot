...
static void setStyle() {
  ...
  lv_style_set_border_color(&popupBox_style, lv_palette_main(LV_PALETTE_DEEP_ORANGE));
  lv_style_set_border_width(&popupBox_style, 5);
  lv_style_init(&style_Clock_text);
  lv_style_set_text_font(&style_Clock_text, &lv_font_montserrat_44); // Use a builtin font, Montserrat size 26
  lv_style_set_text_opa(&style_Clock_text,LV_OPA_COVER);
...
static void buildStatusBar() {
  ...
  lv_obj_t *statusBar = lv_obj_create(home_screen);
  ...
  lv_obj_set_size(settingBtn, 26, 26);
  lv_obj_align(settingBtn, LV_ALIGN_LEFT_MID, 0, 0);
...
static void buildBody() {
  ...
  lv_obj_t *bodyScreen = lv_obj_create(home_screen);
  ...
  //lv_obj_t *Clock = lv_label_create(bodyScreen);
  Clock = lv_label_create(bodyScreen);
  //lv_label_set_text(Clock, "Your Contents");
  lv_obj_add_style(Clock, &style_Clock_text, 0);
  lv_obj_center(Clock);

  dayOfWeek = lv_label_create(bodyScreen);
  //lv_obj_add_style(dayOfWeek, &style_Clock_text, 0);
  lv_obj_align(dayOfWeek, LV_ALIGN_CENTER, 0, -40);

  date = lv_label_create(bodyScreen);
  //lv_obj_add_style(date, &style_Clock_text, 0);
  lv_obj_align(date, LV_ALIGN_CENTER, 0, 40);
}

static void buildSettings() {
  //changed
  settings = lv_obj_create(home_screen);
...
static void buildPWMsgBox() {
  ...
  mboxConnect = lv_obj_create(home_screen);
...
static void popupMsgBox(String title, String msg) {
  ...
  popupBox = lv_obj_create(home_screen);
...
void updateLocalTime() {
  ...
  char date_char[4];
  char week_char[3];
  strftime(hourMin, 9, "%H : %M", &timeinfo);
  strftime(date_char, 4, "%A", &timeinfo);
  strftime(week_char, 3, "%d", &timeinfo);
  String hourMinWithSymbol = String(hourMin);
  String dateWithSymbol = String(date_char);
  String weekWithSymbol = String(week_char);
  // hourMinWithSymbol += "   ";
  // hourMinWithSymbol += LV_SYMBOL_WIFI;
  lv_label_set_text(Clock, hourMinWithSymbol.c_str());
